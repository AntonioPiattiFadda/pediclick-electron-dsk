...
export const DeliveryProductSelector = () => {
  const {
    selectedProduct,
    setSelectedProduct,
  ...
  const selectedProductPresentation = useMemo(() => {
    return (
      productPresentations.find(
        (pp) => pp.product_presentation_id === productPresentationId
      ) || null
    );
  ...
  const selectedStock = useMemo(() => {
    ...
    return (
      selectedLot.stock.find((stock) => stock.stock_id === selectedStockId) ??
      null
    );
          ...
          <div className="flex flex-col gap-1 col-span-8 ">
            ...
            <ProductPresentationSelectorRoot
              ...
              onChange={(value) => {
                ...
                const firstStockOfFirstLotId =
                  value?.lots?.[0]?.stock?.[0]?.stock_id || null;
